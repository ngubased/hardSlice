...
#   HTTP_PORT=8787 ./scripts/setup-ufw-shredstream.sh 8001
#
# Optional lock-down (instead of open-to-world HTTP):
#   HTTP_ALLOW_FROM=203.0.113.50 ./scripts/setup-ufw-shredstream.sh 8001
#
...
# Empty = allow HardSlice HTTP from anywhere (Bearer AUTH_TOKEN still required).
# Set to an IP to restrict the control API to that source only.
...
if [[ ! "$HTTP_PORT" =~ ^[0-9]+$ ]] || (( HTTP_PORT < 1024 || HTTP_PORT > 65535 )); then
  echo "error: HTTP port must be 1024–65535 (got: $HTTP_PORT)" >&2
  exit 1
fi
...
if [[ -n "$HTTP_ALLOW_FROM" ]]; then
  ...
  echo "==> Allowing HardSlice HTTP ${HTTP_PORT}/tcp from anywhere…"
  echo "    (Arm/status still need Bearer AUTH_TOKEN — no wallet secrets over this API.)"
  $SUDO ufw allow "${HTTP_PORT}/tcp" comment "hardSlice control API"
fi
...
echo "  2) Also open ${UDP_PORT}/udp and ${HTTP_PORT}/tcp in your cloud security group if any."
echo "  3) Start hardSlice with UDP_PORT=${UDP_PORT} HTTP_PORT=${HTTP_PORT} (binds 0.0.0.0)."
echo "  4) Confirm shreds:  sudo tcpdump -n -i any udp port ${UDP_PORT} -c 5"
echo "  5) Confirm HTTP:    curl -sS http://${PUBLIC_IP:-127.0.0.1}:${HTTP_PORT}/health"
echo
